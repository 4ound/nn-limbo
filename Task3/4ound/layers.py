...
def softmax(predictions):
    ...
    maxs = np.max(predictions, axis=axis)
    if predictions.ndim == 2:
        maxs = maxs.reshape(-1, 1)
    probabilities = np.exp(predictions - maxs)

    sums = np.sum(probabilities, axis=axis)
    if predictions.ndim == 2:
        sums = sums.reshape(-1, 1)
    probabilities = np.divide(probabilities, sums)
...
def cross_entropy_loss(probs, target_index):
    ...
    Returns:
      loss: single value
    '''
    return -np.log(probs[target_index]) if probs.ndim == 1 else \
        np.sum(-np.log(probs[np.arange(probs.shape[0]), target_index])) / probs.shape[0]
...
class MaxPoolingLayer:
    ...
    def forward(self, X):
        ...
        for y in range(out_height):
            for x in range(out_width):
                current = X[:, y * self.stride: y * self.stride + self.pool_size,
    ...
    def backward(self, d_out):
        ...
        for y in range(out_height):
            for x in range(out_width):
                ...
                current_X = self.X[:, y * self.stride: y * self.stride + self.pool_size,
                            x * self.stride: x * self.stride + self.pool_size, :]
                mask = (current_X == np.max(current_X, axis=(1, 2))[:, np.newaxis, np.newaxis, :])
                result[:, y * self.stride: y * self.stride + self.pool_size,
                x * self.stride: x * self.stride + self.pool_size, :] += grad * mask
